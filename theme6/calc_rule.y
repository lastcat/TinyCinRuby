...
rule
	...
	declaration:RESERVED declarator_list ';'{result = [val[0],val[1]]}
	...
	function_definition:RESERVED declarator '(' parameter_type_list ')' compound_statement{result = [[val[0],val[1]],val[3],val[5]]}
	...
	parameter_declaration:RESERVED declarator{result = [val[0],val[1]]}
	...
	compound_statement:'{'declaration_list statement_list'}'{result = [val[1],val[2]]}
	...
	postfix_expr:primary_expr{result = val[0]}
				|IDENTIFIER '('argument_expression_list')' {result = val[2].unshift(val[0])}
				
	primary_expr:IDENTIFIER {result = val[0]}
